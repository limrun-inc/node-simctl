...
class Simctl {
  ...
  /**
   * Initialize the Limrun client.
   * @returns {Promise<void>}
   */
  async initializeLimClient() {
    if (this.lim) {
      return;
    }
    const instanceId = process.env.LIMRUN_INSTANCE_ID;
    if (!instanceId) {
      throw new Error('LIMRUN_INSTANCE_ID is not set');
    }
    const limrun = new Limrun({ apiKey: process.env.LIMRUN_API_KEY });
    const instance = await limrun.iosInstances.get(instanceId);
    if (!instance.status.endpointWebSocketUrl) {
      throw new Error('endpointWebSocketUrl is not set');
    }
    this.lim = await Ios.createInstanceClient({
  ...
  async exec (subcommand, opts) {
    await this.initializeLimClient();
    let {
      args = [],
      env = {},
      asynchronous = false,
    ...
    try {
      console.log("ignoring execOpts for now", execOpts);
      const execution = this.lim.simctl(args);
