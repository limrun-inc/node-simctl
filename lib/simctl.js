...
import listappsCommands from './subcommands/listapps';
...
import lsofCommands from './subcommands/lsof';
 ...
 * @property {string?} [limInstanceApiUrl] - The lim instance API URL to use.
 * @property {string?} [limInstanceToken] - The lim instance token to use.
 */
class Simctl {
  /** @type {XCRun} */
  xcrun;
  ...
  /**
   * The main Limrun instance client for the simctl instance.
   * Use getLimClient() to access it.
   * @type {Ios.InstanceClient}
   * @private
   */
  _lim;
  ...
  /*
  * @returns {Promise<Ios.InstanceClient>}
  */
  async getLimClient(createNew = false) {
    if (this._lim && !createNew) {
      return this._lim;
    }
    if (createNew) {
      return await Ios.createInstanceClient({
   ...
   * @return {Promise<TExecOpts extends TAsyncOpts ? Ios.SimctlExecution : { code: number; stdout: Buffer; stderr: Buffer }>}
   * Either the result of Limrun's `simctl` execution or
   * the result of the execution depending of `opts.asynchronous` value.
  ...
  async exec (subcommand, opts) {
    ...
    try {
      // We create a new client for async requests so that they don't
      // block the main websocket traffic.
      const lim = await this.getLimClient(asynchronous);
      const execution = lim.simctl(args, { disconnectOnExit: asynchronous });
      // @ts-ignore We know what we are doing here
      return asynchronous ? execution : await execution.wait();
    } catch (e) {
  ...
  listOpenFiles = lsofCommands.lsof;
}
