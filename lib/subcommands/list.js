...
commands.getDevicesByParsing = async function getDevicesByParsing (platform) {
  ...
  while ((match = deviceSectionRe.exec(stdout.toString('utf-8')))) {
    matches.push(match);
  }
...
commands.getDevices = async function getDevices (forSdk, platform) {
  ...
  try {
    ...
    for (let [sdkName, entries] of _.toPairs(JSON.parse(stdout.toString('utf-8')).devices)) {
...
commands.getRuntimeForPlatformVersionViaJson = async function getRuntimeForPlatformVersionViaJson (
  ...
  for (const {version, identifier, name} of JSON.parse(stdout.toString('utf-8')).runtimes) {
...
commands.getDeviceTypes = async function getDeviceTypes () {
   ...
   */
  try {
    const deviceTypes = JSON.parse(stdout.toString('utf-8').trim());
...
commands.list = async function list () {
  ...
  try {
    return JSON.parse(stdout.toString('utf-8').trim());
  } catch (e) {
